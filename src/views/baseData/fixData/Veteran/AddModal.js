...
import { CreateVeteran,GetVeteran } from '@store/slices/fixData'
...
export default function LicenseModal({ IsAddModal, SetIsAddModal }) {
  ...
  const onSubmit = (data) => {
    ...
    dispatch(CreateVeteran(payload)).then(() => {
      dispatch(GetVeteran()).then(() => {
  ...
  return (
    <Modal size='lg' isOpen={IsAddModal} toggle={toggle}>
      <ModalHeader toggle={toggle}> اضافه کردن وضعیت ایثارگری</ModalHeader>
      <ModalBody>
                  ...
                  render={({ field }) => {
                    ...
                    return (
                      <Select
                        ...
                        className={errors.Quota ? 'is-invalid' : ''}
                      />
                    )
                  }}
                />
                {errors.Quota && <FormFeedback className='d-block'>{errors.Quota.message}</FormFeedback>}
              </div>
            </Col>
            {/* دکمه ثبت */}
            <Col lg={12}>
