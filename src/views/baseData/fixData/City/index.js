...
import {GetCity,getProvince   } from '@store/slices/fixData'
...
export default function index() {
  ...
  const rowsWithIndex = store.City.items?.map((item, i) => ({
  ...item,
  ...
  const dataGridData = {
    columns: [
      ...
      { dataField: 'title' , caption: 'اسامی شهرها ' },
      { dataField: 'province.title' , caption: 'اسامی استان ها ' },
      {
        caption: 'عملیات ',
        type: 'buttons',
  ...
  useEffect(() => {
    dispatch(GetCity()).then(()=> dispatch(getProvince()))
  }, [])
