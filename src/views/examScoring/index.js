...
import { GetAllExamScroring } from '@store/slices/examScroing'
...
export default function index() {
  const store = useSelector((state) => state.examScroing)
  ...
  const rowsWithIndex = store.Scores.items?.map((item, i) => ({
    ...item,
  ...
  const dataGridData = {
    columns: [
      ...
      { dataField: 'ruleTypeTitle', caption: 'عنوان نوع قانون' },
      { dataField: 'ruleType', caption: 'کد نوع قانون' },
      { dataField: 'scoringTypeTitle', caption: 'عنوان نوع امتیازدهی' },
      { dataField: 'scoringType', caption: 'کد نوع امتیازدهی' },
      { dataField: 'scoreValue', caption: 'مقدار امتیاز' },
      { dataField: 'condition', caption: 'شرط' },
      { dataField: 'extraData', caption: 'اطلاعات اضافی' },
      {
        caption: 'عملیات ',
        type: 'buttons',
    ...
    <Row>
      <Col lg={12}>
        <p className='route-base-color'>
          ...
          <span className='route-caption'>مدیریت پاسخنامه </span> /{' '}
          <span className='route-caption'>امتیاز دهی آزمون</span>
        </p>
      </Col>
