...
export default function AddModal({ IsAddModal, SetIsAddModal }) {
  ...
  const onSubmit = (data) => {
    const payload = {
      applicantId: 0, // می‌تونی اینجا مقدار دهی کنی
      applicantNationalCode: data.applicantNationalCode,
      ...
      <ModalBody>
        <Form onSubmit={handleSubmit(onSubmit)}>
          <Row>
            {/* کد ملی */}
            <Col lg={6}>
              <Label for='applicantNationalCode'>کد ملی</Label>
              <Controller
                name='applicantNationalCode'
                control={control}
                render={({ field }) => (
                  <Input {...field} invalid={!!errors.applicantNationalCode} placeholder='کد ملی را وارد کنید' />
                )}
              />
              {errors.applicantNationalCode && (
                <FormFeedback className='d-block'>{errors.applicantNationalCode.message}</FormFeedback>
              )}
            </Col>
            ...
            <Col lg={6}>
              ...
              <Controller
                ...
                control={control}
                render={({ field }) => (
                  <Input {...field} invalid={!!errors.applicantDescription} placeholder='توضیحات را وارد کنید' />
                )}
              />
